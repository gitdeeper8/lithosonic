"""
LITHO-SONIC Advanced LSI Model
...
class PhysicsAwareModel:
    ...
    def __init__(self):
        ...
        self.stats = {'calls': 0, 'rejected': 0, 'adjusted': 0, 'flagged': 0}
    ...
    def check_constraints(self, params: Dict) -> List[Dict]:
        ...
        for constraint in self.constraints:
            try:
                ...
                if eval(constraint.condition, {"__builtins__": {}}, params):
                    violations.append({
    ...
    def compute(self, params: Dict) -> Dict:
        ...
        self.stats['calls'] += 1
        self.stats['total_time'] = self.stats.get('total_time', 0) + (time.time() - start)
...
class AdaptiveHybrid:
    ...
    def _print_analysis(self, params: Dict, result: Dict):
        ...
        print(f"   LSI: {result['lsi']:.3f}" if result['lsi'] else f"   LSI: INVALID")
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'fast_path_percent': self.stats['fast_path'] / self.stats['total'] * 100,
            'physics_path_percent': self.stats['physics_path'] / self.stats['total'] * 100,
            'false_positive_rate': self.stats['false_positives'] / self.stats['total'] * 100,
            'false_negative_rate': self.stats['false_negatives'] / self.stats['total'] * 100,
...
def run_comprehensive_test():
    ...
    print("=" * 80)
    print("🧪 LITHO-SONIC Advanced Model Test Suite")
    print("=" * 80)
    ...
    print("\n📈 Testing Linear Core (438k samples/sec)")
    print("-" * 50)
    for name, params in test_cases[:5]:
        lsi = linear.compute(params)
        alert = linear.get_alert(lsi)
        print(f"  {name:30} LSI={lsi:.3f} [{alert}]")
    ...
    for name, params in test_cases:
        ...
        if result['lsi'] is not None:
            print(f"  {name:30} LSI={result['lsi']:.3f} [{result['alert']}]  "
                  f"adj={result['adjustment_factor']:.2f}")
        else:
            print(f"  {name:30} REJECTED - {result['violations'][0]['message']}")
    ...
    for name, params in test_cases:
        ...
        if result['lsi'] is not None:
            ...
            print(f"  {name:30} INVALID DATA")
    
    print("\n📊 Performance Summary")
    print("-" * 50)
    print(json.dumps(hybrid.get_stats(), indent=2))
